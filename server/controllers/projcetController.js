...
export const createProject = async(req, res) =>{
    try {
       ...
       if(!workspace.members.some((member) =>member.userId === userId && member.role === "ADMIN")){
        return res.status(403).json({message: "You don't have permession to create projects in the workspace"})
       }
    ...
    try {
       ...
       if(!workspace.members.some((member)=> member.userId ===userId && member.role === "ADMIN")){
         ...
         if(!project){
            ...
            return res.status(403).json({message: "You don't have permession to update projects in the workspace"});
        }
...
export const addMember = async(req, res) =>{
    try {
        ...
        if(!project){
            return res.status(404).json({message: "Prohect not found"});
        }
